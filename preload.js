...
contextBridge.exposeInMainWorld('flock', {
  ...
  onSaveView: (handler) => ipcRenderer.on('save-view', () => handler()),
  onLoadView: (handler) => ipcRenderer.on('load-view', (_e, name) => handler(name)),
  onDeleteView: (handler) => ipcRenderer.on('delete-view', (_e, name) => handler(name)),
